...
for (let i = 0; i < input.length; i++) {
    let correct = true
    for (let k = 0; k < regole.length; k++) {
        if (input[i].includes(regole[k][0]) && input[i].includes(regole[k][1])) {
            beforePage = input[i].indexOf(regole[k][0])
            afterPage = input[i].indexOf(regole[k][1])
...
right.forEach(el =>{
    indexToExtraxt = (el.length - 1) / 2
    // console.log(indexToExtraxt)
    sumArray.push(el[indexToExtraxt])
})
