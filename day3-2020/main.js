...
// trasformo la stringa in array di 3 el
function sintaxData(string) {
    return string.split(' ')
}

for (let i = 0; i < passwords.length; i++) {

    scomposedPassword = sintaxData(passwords[i])
    // Calcolo del numero di pattern nella stringa
    letterToSearch = scomposedPassword[1].replace(':', '')
    numberOfLetters = scomposedPassword[scomposedPassword.length - 1].split(letterToSearch).length - 1
    // console.log(numberOfLetters)

    // Estraggo i numeri min e max

    minMax = scomposedPassword[0].split('-')


    if (numberOfLetters >= minMax[0] && numberOfLetters <= minMax[1]) {
        counter++
    }
}
